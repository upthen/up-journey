...
async function load() {
  ...
  loading.value = false // 先渲染出卡片，观察器才能拿到 .reveal 节点
  await nextTick()
...
<template>
  <div>
    ...
    <main class="wrap page-pad">
      ...
      <div v-else-if="loading" class="loading-dots"><i></i><i></i><i></i></div>
      ...
      <template v-else>
        ...
        <p v-if="groups.length" class="tl-end-note">
